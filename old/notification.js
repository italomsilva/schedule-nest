...
Parse.Cloud.define('v1-send-push-notification', async (req) => {
  return await sendPushNotification(
    req.params.userId,
    req.params.dynamicNotificationKey,
...
Parse.Cloud.define(
  ...
  async (req) => {
    ...
    const notificationsCount = await queryNotifications.count({
      useMasterKey: true,
    });
...
async function sendPushNotification(userId, dynamicNotificationKey, variables) {
  ...
  const dynamicNotification = await queryDynamicNotification.first({
    useMasterKey: true,
  });
  ...
  const subtitle = replaceVariables(
    dynamicNotification.get('subtitle'),
    variables,
  );
  ...
  const message = {
    data: {
      notification: JSON.stringify(
        formatNotification(savedNotification.toJSON()),
      ),
    },
    notification: {
      title: title,
...
function replaceVariables(text, variables) {
  ...
  if (newText.includes('{') || newText.includes('}'))
    throw 'INVALID_VARIABLES' + newText;
